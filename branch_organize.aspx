...
<asp:content id="Content0" contentplaceholderid="PlaceHolderAdditionalPageHead" runat="server">
   ...
   //杭州一般党员信息
    function hzpartygeneralf(arr){
        ...
        //上海顶部
        //第一行
        $(".sh .content_organize_oneul").find(".img_people").attr({"src":shpartysum[0].FileRef,"ordernumber":"0"});
        $(".sh .content_organize_oneul").find(".mlk_name").html(shpartysum[0].name);
        $(".sh .content_organize_oneul").find(".mlk_name_ziwei").html(shpartysum[0].branchposition);
        //第二行循环
        var shtoptwo="";
        for(var i=1;i<5;i++){
           shtoptwo+='<li class="line2"><img src="img/organize/71.png" class="line2_img"><span class="mlk_personal float_father">'+
                     '<img src="'+shpartysum[i].FileRef+'" class="img_people_2" place="shpartysump" ordernumber="'+i+'">'+
                      '<div class="mlk_name">'+shpartysum[i].name+'</div><div class="mlk_name_ziwei">'+
                       shpartysum[i].branchposition+'</div>'+
                       '</span><img src="img/organize/72.png" class="line2_img"></li>'
        }
        $(".sh .content_organize_twoul").html(shtoptwo);

        //第三行循环
        var shtopthree="";
        for(var i=5;i<shpartysum.length;i++){
           shtopthree+='<li class="img_people_3"><span class="mlk_personal float_father">'+
                         '<img place="shpartysump" src="'+shpartysum[i].FileRef+'" ordernumber="'+i+'">'+
                         '<div class="mlk_name">'+shpartysum[i].name+'</div>'+
                         '<div class="mlk_name_ziwei">'+shpartysum[i].branchposition+'</div></span></li>'
        }
        $(".sh .content_organize_threeul").html(shtopthree);

        //上海轮播
        var shbotswiper="";
        for(var i=0;i<shpartygeneral.length;i++){
           shbotswiper+='<div class="swiper-slide swiper-no-swiping"><div class="float_father">'+
                         '<img place="shpartygeneralp" src="'+shpartygeneral[i].FileRef+'" ordernumber="'+i+'">'+
                          '<div class="mlk_swiper_name">'+shpartygeneral[i].name+'</div>'+
                          '</div></div>'
        }
        $(".swiper-container-sh .swiper-wrapper").html(shbotswiper);

        //杭州顶部
        //第一行
        $(".hz .content_organize_oneul").find(".img_people").attr({"src":hzpartysum[0].FileRef,"ordernumber":"0"});
        $(".hz .content_organize_oneul").find(".mlk_name").html(hzpartysum[0].name);
        $(".hz .content_organize_oneul").find(".mlk_name_ziwei").html(hzpartysum[0].branchposition);
        //第二行循环
        var hztoptwo="";
        for(var i=1;i<5;i++){
           hztoptwo+='<li class="line2"><img src="img/organize/71.png" class="line2_img"><span class="mlk_personal float_father">'+
                     '<img place="hzpartysump" src="'+hzpartysum[i].FileRef+'" class="img_people_2"  ordernumber="'+i+'">'+
                      '<div class="mlk_name">'+hzpartysum[i].name+'</div>'+
                      '<div class="mlk_name_ziwei">'+hzpartysum[i].branchposition+'</div></span>'+
                      '<img src="img/organize/72.png" class="line2_img"></li>'
        }
        $(".hz .content_organize_twoul").html(hztoptwo);

        //第三行循环
        var hztopthree="";
        for(var i=5;i<hzpartysum.length;i++){
           hztopthree+='<li class="img_people_3"><span class="mlk_personal float_father">'+
                         '<img place="hzpartysump" src="'+hzpartysum[i].FileRef+'" ordernumber="'+i+'">'+
                         '<div class="mlk_name">'+hzpartysum[i].name+'</div>'+
                         '<div class="mlk_name_ziwei">'+hzpartysum[i].branchposition+'</div></span></li>'
        }
        $(".hz .content_organize_threeul").html(hztopthree);

        //杭州轮播
        var hzbotswiper="";
        for(var i=0;i<hzpartygeneral.length;i++){
           hzbotswiper+='<div class="swiper-slide swiper-no-swiping"><div class="float_father">'+
                         '<img place="hzpartygeneralp" src="'+hzpartygeneral[i].FileRef+'" ordernumber="'+i+'">'+
                          '<div class="mlk_swiper_name">'+hzpartygeneral[i].name+'</div>'+
                          '</div></div>'
        }
        $(".swiper-container-hz .swiper-wrapper").html(hzbotswiper);
        ...
        $(".float_father").find("img").on("click",function(){
            ...
            function box_same_show(obj){
                //循址显示弹出内容
                var arrindex=$(obj).attr("ordernumber");
                //console.log(arrindex);
                //寻找对应数组
                var arrnameplace=$(obj).attr("place");
                var arrname=[];
                if(arrnameplace=="shpartysump"){
                    arrname=shpartysum
                }else if(arrnameplace=="shpartygeneralp"){
                    arrname=shpartygeneral
                }else if(arrnameplace=="hzpartysump"){
                    arrname=hzpartysum
                }else if(arrnameplace=="hzpartygeneralp"){
                    arrname=hzpartygeneral
                }
                $(".float_detail").find(".float_detail_name").html(arrname[arrindex].name);
                $(".float_detail").find(".detail_date").html(arrname[arrindex].joinTime);
                $(".float_detail").find(".detail_branch").html(arrname[arrindex].locatedbranch);
                $(".float_detail").find(".detail_comjob").html(arrname[arrindex].companyposition);
                $(".float_detail").find(".detail_branjob").html(arrname[arrindex].branchposition);
                $(".float_detail").find(".detail_hobby").html(arrname[arrindex].hobby);
                $(".float_detail").find(".detail_skill").html(arrname[arrindex].specialty);
                    ...
                    <ul class="content_organize_oneul">
                        <li class="line1">
                            ...
                            <span class="mlk_personal float_father">
                                <img src="" class="img_people" place="shpartysump" ordernumber="">
                                <div class="mlk_name">
                                </div>
                ...
                <div class="content_organize_one">
                    <ul class="content_organize_oneul">
                        <li class="line1">
                            <img src="img/organize/73.png" class="line1_img">
                            <span class="mlk_personal float_father">
                                <img src="" class="img_people" place="hzpartysump" ordernumber="">
                                <div class="mlk_name">
        ...
        <div class="float_detail">
            ...
            <ul>
                ...
                <li><span class="detail_title">特&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;长:</span> <span
                    class="detail_skill detail_content"></span></li>
            </ul>
        </div>
    </div>
</asp:content>
